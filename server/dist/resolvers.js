...
const resolvers = {
    Query: {
        places: (_, { coordinates: { lat, lng }, pagetoken = null, keyword = '' }, { mapClient }) => __awaiter(void 0, void 0, void 0, function* () {
            const params = {
                key,
                location: `${lat},${lng}`,
        ...
        place: (_, { placeId }, { mapClient }) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        reviews: (_, { placeId, limit = 6, offset = 0 }, { models }) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        getUser: (_, __, { req, models }) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        ratings: (_, { placeId }) => __awaiter(void 0, void 0, void 0, function* () {
            const ratings = yield getRatings_1.default(placeId);
            return { placeId, ratings };
        }),
    },
    Photo: {
        photo_reference: (parent, _, { mapClient }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    Mutation: {
        signup: (_, { user: { email, name, password, profilePic } }, { models, req }) => __awaiter(void 0, void 0, void 0, function* () {
            const doesUserExist = yield models.User.exists({ email });
            if (doesUserExist)
                throw new apollo_server_express_1.UserInputError('Email already exists');
            let imgUrl;
            if (profilePic) {
                const path = path_1.join(os_1.tmpdir(), profilePic.file.filename);
                yield pipeline(profilePic.file.createReadStream(), fs_1.default.createWriteStream(path));
                imgUrl = yield s3_1.uploadImage('prfpics-' + profilePic.file.filename, path);
            }
        ...
        signin: (_, { user: { email, password } }, { models, req }) => __awaiter(void 0, void 0, void 0, function* () {
            const user = yield models.User.findOne({ email });
            if (!user)
                throw new apollo_server_express_1.UserInputError('User with this email does not exist');
        ...
        postReview: (_, { review, files }, { req, models }) => __awaiter(void 0, void 0, void 0, function* () {
            if (!req.session.user)
                throw new apollo_server_express_1.ForbiddenError('You must be signed in to post a review');
            const fileUrls = [];
            if (files === null || files === void 0 ? void 0 : files.length) {
                const resolvedFiles = yield Promise.all(files.map((file) => file.promise));
                while (resolvedFiles.length) {
                    const file = resolvedFiles.pop();
                    const path = path_1.join(os_1.tmpdir(), file.filename);
                    yield pipeline(file.createReadStream(), fs_1.default.createWriteStream(path));
                    const objUrl = yield s3_1.uploadImage(file.filename, path);
                    fileUrls.push(objUrl);
                }
            }
            const newReview = yield models.Review.create(Object.assign(Object.assign({}, review), { images: fileUrls }));
            yield models.Review.populate(newReview, { path: 'user' });
            return newReview;
        }),
        deleteReview: (_, { id, hasImages }, { req, models }) => __awaiter(void 0, void 0, void 0, function* () {
            if (!req.session.user)
                throw new apollo_server_express_1.ForbiddenError('You must be signed');
            let review;
            let images;
            if (hasImages) {
        ...
        logout: (_, __, { req }) => __awaiter(void 0, void 0, void 0, function* () {
